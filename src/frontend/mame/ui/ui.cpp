...
{
	if (mui.machine().options().ui() == emu_options::UI_MENUS)
		return mui.handler_ingame_method(container, state);
	else
		return handler_ingame_old(mui, container, state);
}

//-------------------------------------------------
//  handler_ingame_old - in-game handler takes care
//  of the standard keypresses
//-------------------------------------------------

UINT32 mame_ui_manager::handler_ingame_old(mame_ui_manager &mui, render_container *container, UINT32 state)
{
