...
void info_xml_creator::output_header(std::ostream &out)
{
	...
	// top-level tag
	util::stream_format(out, "<%s build=\"%s\" debug=\""
#ifdef MAME_DEBUG
			"yes"
#else
			"no"
...
void info_xml_creator::output_one(std::ostream &out, driver_enumerator &drivlist, device_type_set *devtypes)
{
	...
	util::stream_format(out, "\t<%s name=\"%s\"", XML_TOP, util::xml::normalize_string(driver.name));
	...
	util::stream_format(out, " sourcefile=\"%s\"", util::xml::normalize_string(start));
	...
	if (clone_of != -1 && !(drivlist.driver(clone_of).flags & machine_flags::IS_BIOS_ROOT))
		util::stream_format(out, " cloneof=\"%s\"", util::xml::normalize_string(drivlist.driver(clone_of).name));
	if (clone_of != -1)
		util::stream_format(out, " romof=\"%s\"", util::xml::normalize_string(drivlist.driver(clone_of).name));
...
void info_xml_creator::output_one_device(std::ostream &out, machine_config &config, device_t &device, const char *devtag)
{
	...
	util::stream_format(out, "\t<%s name=\"%s\"", XML_TOP, util::xml::normalize_string(device.shortname()));
	std::string src(device.source());
	strreplace(src,"../", "");
	util::stream_format(out, " sourcefile=\"%s\" isdevice=\"yes\" runnable=\"no\"", util::xml::normalize_string(src.c_str()));
...
void info_xml_creator::output_rom(std::ostream &out, driver_enumerator *drivlist, const game_driver *driver, device_t &device)
{
	...
	for (type pass : { type::BIOS, type::NORMAL, type::DISK })
	{
		...
		for (tiny_rom_entry const *rom = device.rom_region(); rom && !ROMENTRY_ISEND(rom); ++rom)
		{
			...
			if (name && name[0])
				util::stream_format(out, " name=\"%s\"", util::xml::normalize_string(name));
			if (merge_name)
				util::stream_format(out, " merge=\"%s\"", util::xml::normalize_string(merge_name));
			if (bios_name)
				util::stream_format(out, " bios=\"%s\"", util::xml::normalize_string(bios_name));
			if (!is_disk)
...
void info_xml_creator::output_chips(std::ostream &out, device_t &device, const char *root_tag)
{
	...
	for (device_execute_interface &exec : execute_interface_iterator(device))
	{
		if (strcmp(exec.device().tag(), device.tag()))
		{
			...
			out << "\t\t<chip";
			out << " type=\"cpu\"";
			util::stream_format(out, " tag=\"%s\"", util::xml::normalize_string(newtag.c_str()));
			util::stream_format(out, " name=\"%s\"", util::xml::normalize_string(exec.device().name()));
	...
	for (device_sound_interface &sound : sound_interface_iterator(device))
	{
		if (strcmp(sound.device().tag(), device.tag()) != 0 && sound.issound())
		{
			...
			out << " type=\"audio\"";
			util::stream_format(out, " tag=\"%s\"", util::xml::normalize_string(newtag.c_str()));
			util::stream_format(out, " name=\"%s\"", util::xml::normalize_string(sound.device().name()));
...
void info_xml_creator::output_display(std::ostream &out, device_t &device, machine_flags::type const *flags, const char *root_tag)
{
	...
	for (const screen_device &screendev : screen_device_iterator(device))
	{
		if (strcmp(screendev.tag(), device.tag()))
		{
			...
			util::stream_format(out, "\t\t<display tag=\"%s\"", util::xml::normalize_string(newtag.c_str()));
...
void info_xml_creator::output_input(std::ostream &out, const ioport_list &portlist)
{
	...
	for (auto & elem : control_info)
		if (elem.type != nullptr)
		{
			...
			if (elem.analog)
			{
				util::stream_format(out, "\t\t\t<control type=\"%s\"", util::xml::normalize_string(elem.type));
				if (nplayer > 1)
			...
			else
			{
				...
				util::stream_format(out, "\t\t\t<control type=\"%s%s\"", joys, util::xml::normalize_string(elem.type));
				if (nplayer > 1)
					util::stream_format(out, " player=\"%d\"", elem.player);
				if (elem.nbuttons > 0)
				{
	...
	for (auto &port : portlist)
		for (ioport_field const &field : port.second->fields())
			if (field.type() == type)
			{
				...
				{
					util::stream_format(out, "\t\t\t<%s name=\"%s\" number=\"%u\"", loctag, util::xml::normalize_string(diploc.name()), diploc.number());
					if (diploc.inverted())
						out << " inverted=\"yes\"";
					out << "/>\n";
				}
				...
				{
					util::stream_format(out, "\t\t\t<%s name=\"%s\" value=\"%u\"", innertag, util::xml::normalize_string(setting.name()), setting.value());
...
void info_xml_creator::output_images(std::ostream &out, device_t &device, const char *root_tag)
{
	for (const device_image_interface &imagedev : image_interface_iterator(device))
	{
		if (strcmp(imagedev.device().tag(), device.tag()))
		{
			...
			util::stream_format(out, "\t\t<device type=\"%s\"", util::xml::normalize_string(imagedev.image_type_name()));
			...
			if (imagedev.image_interface() && imagedev.image_interface()[0])
				util::stream_format(out, " interface=\"%s\"", util::xml::normalize_string(imagedev.image_interface()));
			...
			if (loadable)
			{
				...
				out << "\t\t\t<instance";
				util::stream_format(out, " name=\"%s\"", util::xml::normalize_string(name));
				util::stream_format(out, " briefname=\"%s\"", util::xml::normalize_string(shortname));
				out << "/>\n";
		...
		if (devtypes || listed)
		{
			...
			for (auto &option : slot.option_list())
			{
				if (devtypes || (listed && option.second->selectable()))
				{
					...
					{
						util::stream_format(out, "\t\t\t<slotoption name=\"%s\"", util::xml::normalize_string(option.second->name()));
						util::stream_format(out, " devname=\"%s\"", util::xml::normalize_string(dev->shortname()));
...
void info_xml_creator::output_software_list(std::ostream &out, device_t &root)
{
	...
	{
		util::stream_format(out, "\t\t<softwarelist name=\"%s\" status=\"%s\"", util::xml::normalize_string(swlist.list_name().c_str()), (swlist.list_type() == SOFTWARE_LIST_ORIGINAL_SYSTEM) ? "original" : "compatible");
		if (swlist.filter())
			util::stream_format(out, " filter=\"%s\"", util::xml::normalize_string(swlist.filter()));
		out << "/>\n";
	}
}
