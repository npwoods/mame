...
typedef std::unordered_map<std::string,const util::software_info *> softlist_map;
...
void software_list_device::find_approx_matches(const char *name, int matches, const util::software_info **list, const char *interface)
{
	// if no name, return
	...
	for (const util::software_info &swinfo : get_info())
	{
...
void software_list_device::display_matches(const machine_config &config, const char *interface, const char *name)
{
	...
	for (software_list_device &swlistdev : deviter)
	{
		...
		const util::software_info *matches[16] = { nullptr };
...
const util::software_info *software_list_device::find(const char *look_for)
{
...
void software_list_device::parse()
{
	...
	if (filerr == osd_file::error::NONE)
	{
		...
		util::softlist_parser parser(m_file, m_description, m_infolist, errs);
		m_file.close();
		m_errors = errs.str();
	}
	else
...
software_compatibility software_list_device::is_compatible(const util::software_part &swpart) const
{
...
device_image_interface *software_list_device::find_mountable_image(const machine_config &mconfig, const util::software_part &part)
{
...
void software_list_device::internal_validity_check(validity_checker &valid)
{
	...
	for (const util::software_info &swinfo : get_info())
	{
		...
		{
			const util::software_info *match = names.find(swinfo.shortname())->second;
		...
		if (!swinfo.parentname().empty())
		{
			...
			const util::software_info *swinfo2 = find(swinfo.parentname().c_str());
		...
		for (const util::software_part &part : swinfo.parts())
		{
			...
			for (const util::rom_entry &data : part.romdata())
			{
				if (!data.hashdata().empty())
				{
					...
					if (!hashes.from_internal_string(data.hashdata().c_str()))
						osd_printf_error("%s: %s has rom '%s' with an invalid hash string '%s'\n", filename(), swinfo.shortname().c_str(), data.name().c_str(), data.hashdata().c_str());
				}
			}
		}
	}
