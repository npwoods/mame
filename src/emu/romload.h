...
#define ROM_START(name)                             static const util::rom_entry ROM_NAME(name)[] = {
...
public:
	...
	void load_software_part_region(device_t &device, software_list_device &swlist, const char *swname, const util::rom_entry *start_region);

private:
	void determine_bios_rom(device_t &device, const char *specbios);
	void count_roms();
	void fill_random(UINT8 *base, UINT32 length);
	void handle_missing_file(const util::rom_entry *romp, std::string tried_file_names, chd_error chderr);
	...
	int open_rom_file(const char *regiontag, const util::rom_entry *romp, std::string &tried_file_names, bool from_list);
	int rom_fread(UINT8 *buffer, int length, const util::rom_entry *parent_region);
	int read_rom_data(const util::rom_entry *parent_region, const util::rom_entry *romp);
	void fill_rom_data(const util::rom_entry *romp);
	void copy_rom_data(const util::rom_entry *romp);
	void process_rom_entries(const char *regiontag, const util::rom_entry *parent_region, const util::rom_entry *romp, device_t *device, bool from_list);
	chd_error open_disk_diff(emu_options &options, const util::rom_entry *romp, chd_file &source, chd_file &diff_chd);
	void process_disk_entries(const char *regiontag, const util::rom_entry *parent_region, const util::rom_entry *romp, const char *locationtag);
