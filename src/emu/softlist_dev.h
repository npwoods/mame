...
public:
	...
	void find_approx_matches(const char *name, int matches, const util::software_info **list, const char *interface);
	void release();
	software_compatibility is_compatible(const util::software_part &part) const;
	...
	static device_image_interface *find_mountable_image(const machine_config &mconfig, const util::software_part &part);
...
private:
	...
	std::string						m_errors;
	std::list<util::software_info>	m_infolist;
};
