...
static MACHINE_CONFIG_FRAGMENT(coco12_standard_cartridge)
	...
	MCFG_COCO_CARTRIDGE_INSTALLRH_CB([owner](uint16_t addrstart, uint16_t addrend, read8_delegate rhandler) { downcast<coco12_state *>(owner)->cart_install_read_handler(addrstart, addrend, rhandler); })
	MCFG_COCO_CARTRIDGE_INSTALLWH_CB([owner](uint16_t addrstart, uint16_t addrend, write8_delegate whandler) { downcast<coco12_state *>(owner)->cart_install_write_handler(addrstart, addrend, whandler); })
MACHINE_CONFIG_END
...
static MACHINE_CONFIG_START( coco, coco12_state )
	...
	MCFG_FRAGMENT_ADD(coco12_standard_cartridge)
